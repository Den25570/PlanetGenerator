...
#include "../Public/PlanetGenerator.h"
...
{
	Icosahedron icosahedron = *(WorldGeometry::generateIcosahedron());

	/*action.executeSubaction(function(action)
	{*/
	for (Face* face : icosahedron.faces)
	{
		FVector p0 = *(icosahedron.nodes[face->nodes->at(0)]->position) * 1000;
		FVector p1 = *(icosahedron.nodes[face->nodes->at(1)]->position) * 1000;
		FVector p2 = *(icosahedron.nodes[face->nodes->at(2)]->position) * 1000;

		FVector center = (p0 + p1 + p2) / 3;
		float radius = fmax(FVector::Distance(center, p0), fmax(FVector::Distance(center, p1), FVector::Distance(center, p2)));
		face->bounding_sphere = new Sphere(center, radius);
		face->children = {};
	}
	/*});*/
	...
	{*/
		for (Tile* tile : tiles)
		{
			maxDistanceFromOrigin = fmax(maxDistanceFromOrigin, FVector::Distance(tile->bounding_sphere->center, FVector::ZeroVector) + tile->bounding_sphere->radius);

			bool parentFound = false;
			for (Face* face : icosahedron.faces)
			{
				float distance = FVector::Distance(tile->bounding_sphere->center, face->bounding_sphere->center) + tile->bounding_sphere->radius;
				if (distance < face->bounding_sphere->radius)
				{
	...
	action.executeSubaction(function(action)
	{
		...
		for (var i = 0; i < icosahedron.faces.length; ++i)
		{
			...
			rootPartition.partitions.push(new SpatialPartition(face.bounding_sphere, [], face.children));
			delete face.bounding_sphere;
...
function generatePlanetPartition(tiles, action)
{
	...
	action.executeSubaction(function(action)
	{
		for (var i = 0; i < icosahedron.faces.length; ++i)
		{
			...
			face.bounding_sphere = new THREE.Sphere(center, radius);
			face.children = [];
		}
	});
	...
	action.executeSubaction(function(action)
	{
		...
		{
			var tile = tiles[i];
			maxDistanceFromOrigin = Math.max(maxDistanceFromOrigin, tile.bounding_sphere.center.length() + tile.bounding_sphere.radius);

			var parentFound = false;
			for (var j = 0; j < icosahedron.faces.length; ++j)
			{
				var face = icosahedron.faces[j];
				var distance = tile.bounding_sphere.center.distanceTo(face.bounding_sphere.center) + tile.bounding_sphere.radius;
				if (distance < face.bounding_sphere.radius)
				{
...
void generatePlanetBiomes(std::vector<Tile> tiles, float planetRadius/*, action*/)
{
	for (Tile tile : tiles)
	{
		float elevation = fmax(0.0f, tile.elevation);
		float latitude = abs(tile.position.Y / planetRadius);
...
{
	std::vector<std::vector<int>> corners = mesh.faces;
	std::vector<std::vector<int>> borders = mesh.edges;
	std::vector<Node> tiles = mesh.nodes;
	...
	action.executeSubaction(function(action)
	{
		for (var i = 0; i < tiles.length; ++i)
		{
			...
			tile.bounding_sphere = new THREE.Sphere(tile.averagePosition, maxDistanceToCorner);
		}
	});
